...
const Navbar = () => {
  ...
  //       if (user.role_id === 3) {
  //         setShowAdminPanel(true); // Show "Admin Panel" button
  //       }
  //     } catch (error) {
  ...
  if (userCookie) {
    try {
      const user = JSON.parse(userCookie);
      roleId = user.role_id; // Extract role_id
    } catch (error) {
      console.error("Error parsing user cookie:", error);
    }
  }
  ...
  const nav = [
    ...
    ...(authState.isAuthenticated
      ? [{ label: "Pyramid Profit", route: "/referral-tree" }]
      : []),
    ...(roleId === 3 ? [{ label: "Admin Panel", route: "https://dashboard-new.tamkeen.center", external: true }] : []), // ✅ Opens in a new tab
  ];
