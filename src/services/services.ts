...
export const registerVendor = async (vendorData: VendorRegisterData): Promise<Response> => {
  const formData = new FormData();

  formData.append("name", vendorData.name);
  formData.append("password", vendorData.password);
  formData.append("store_name", vendorData.store_name);
  formData.append("trn", vendorData.trn);
  formData.append("location", vendorData.location);
  formData.append("store_phone", vendorData.store_phone);
  formData.append("store_email", vendorData.store_email);
  formData.append("marketing_consent", vendorData.marketingConsent ? "true" : "false");

  if (vendorData.documents) {
    Array.from(vendorData.documents).forEach((file) => {
      formData.append("documents", file);
    });
  }

  const { data } = await apiClient.post<Response>("/join-as-vendor", formData, {
    headers: {
